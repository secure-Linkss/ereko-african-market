...
export default function RecipesIndexPage() {
  ...
  const { data: recipes, isLoading } = useRecipes(12);
    ...
    <main className="flex-1 max-w-7xl mx-auto w-full p-4 md:p-8 space-y-12">
      ...
      <div className="flex gap-3 overflow-x-auto pb-2 scrollbar-hide justify-center flex-wrap">
        {CATEGORIES.map((cat) => (
          <button
            key={cat}
            className="px-5 py-2 rounded-full border border-border hover:border-primary hover:bg-primary/5 whitespace-nowrap font-medium transition-colors text-sm"
          >
            {cat}
          </button>
        ))}
      ...
      {isLoading ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {filtered.map((recipe: any) => (
            <Link key={recipe.id} href={`/${locale}/recipes/${recipe.slug}`}>
                ...
                <CardContent className="p-6 flex-1 flex flex-col">
                  ...
                  <div className="flex items-center justify-between text-sm text-muted-foreground border-t border-border pt-4">
                    <span className="flex items-center gap-1">
                      <ChefHat className="w-3.5 h-3.5" /> Medium
                    </span>
                    {recipe.servings && (
                      <span className="flex items-center gap-1">
